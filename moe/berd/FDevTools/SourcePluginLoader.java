    ...
    public boolean compilePlugin(File dir, File output) throws Exception {
        if (output.isFile()) {
            ...
            if (!output.mkdirs()) {
                ...
                CompilationTask task = this.compiler.getTask(null, this.filemanager, null, Arrays.asList("-d",
                        output.getAbsolutePath(),
                        "-encoding",
                        "utf-8"), null, this.filemanager.getJavaFileObjects((File[]) files.toArray(new File[files.size()])));
                return task.call();
            }
        }
    }
